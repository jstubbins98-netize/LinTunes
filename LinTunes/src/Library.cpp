...
FileMetadata Library::readMetadata(const std::string& path) {
    ...
    std::transform(meta.file_format.begin(), meta.file_format.end(),
                   meta.file_format.begin(), ::tolower);
...
bool Library::importFile(const std::string& path, Track& out_track) {
    ...
    if (db_.trackExists(path)) {
        // Already in library — return existing
        auto all = db_.searchTracks(path);
        if (!all.empty()) out_track = all[0];
        return true;
    }
    ...
    Track t;
    t.file_path    = path;
    t.title        = meta.title.empty() ? fs::path(path).stem().string() : meta.title;
