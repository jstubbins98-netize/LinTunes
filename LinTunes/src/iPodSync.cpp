...
bool iPodSync::connect(const std::string& mount_point) {
    ...
    if (connected_uuid_.empty()) {
        disconnect();
        last_error_ = "The iPod does not provide a stable device identity.";
        return false;
    }
    std::string ignoredPath;
    if (!resolveWholeDisk(mount_point_, ignoredPath, mounted_device_number_)) {
        disconnect();
        last_error_ = "Could not resolve the iPod's physical disk identity.";
        return false;
    }
    return true;
}
...
bool iPodSync::modelIdentifiedForReset(const std::string& model) {
    return !model.empty();
}
...
bool iPodSync::resetMusicDatabase(const std::string& display_name) {
    ...
    if (!modelIdentifiedForReset(model)) {
    ...
    if (!resolveWholeDisk(mount, currentDevicePath, currentDeviceNumber) ||
