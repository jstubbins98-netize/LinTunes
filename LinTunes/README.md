# LinTunes User Manual

LinTunes is a full-featured music manager for Linux, designed to work and feel like iTunes. It manages your music library, plays audio, syncs to iPods, rips and burns CDs, and supports the Apple SuperDrive.
...
The easiest way to install LinTunes is with the included build script. It detects your Linux distribution, installs all required packages, and compiles the program in one step.
...
Supported distributions: Ubuntu/Debian, Fedora/RHEL, Arch Linux.
...
When LinTunes starts for the first time, the library will be empty and a welcome screen is shown with a large **+ Add Music** button. The library database is created automatically at `~/.local/share/lintunes/library.db`. Nothing is imported automatically — you choose what goes in your library.
...
LinTunes never moves or copies your files — it adds references to wherever your audio files already live on disk.
...
- **Add Folder…** — opens a folder chooser. LinTunes scans the selected folder and all sub-folders recursively, importing every audio file it finds.
...
Drag audio files or folders from your file manager directly onto the track list. LinTunes accepts them and imports them in the background. You can drop multiple files and folders at the same time.

### What happens during import

- LinTunes reads the embedded tags (title, artist, album, genre, year, track number, disc number, composer, album artist, BPM, comment) using TagLib.
...
| **Remove from Library** | Removes the track(s) from LinTunes but leaves the audio file on disk |
...
LinTunes supports all classic iPods (iPod classic, iPod mini, iPod nano, iPod photo) using the libgpod library.
...
### Step 2 — Connect in LinTunes

Go to **Device → Connect iPod…**

LinTunes automatically scans `/proc/mounts`, `/media`, `/mnt`, and `/run/media` for a directory containing an `iPod_Control` or `iTunes_Control` folder. If exactly one iPod is found, it connects automatically. If multiple are found, a dialog lets you choose which one.
...
4. When all files are copied, LinTunes writes the iPod database automatically.
...
1. Install `idevicerestore` and `idevice_id` using your distribution's package
   manager.
2. Obtain an Apple-signed IPSW for the exact iPod touch model. LinTunes does not
...
7. Do not unplug the iPod until LinTunes reports that the restore finished.

LinTunes records the selected UDID, verifies that same device again immediately
...
binds the destructive write to the iPod selected in LinTunes. LinTunes refuses
...
iPod. A full firmware-partition restore must be performed outside LinTunes with
...
1. Mount and connect the disk-mode iPod normally in LinTunes.
2. Choose **Device → Erase and Reset Music Database…**.
3. Verify the displayed mount point.
4. Type `ERASE MUSIC` exactly and confirm.

LinTunes identifies the model through libgpod, deletes only the iPod control
...
Immediately before deleting anything, LinTunes reopens the iPod database and
checks that the mount point still refers to the originally connected device. It
verifies the physical block-device identity, iPod UUID, and model number. If the
iPod was unplugged, remounted as another disk, or swapped after confirmation,
...
- Connect exactly one iPod touch before opening the restore workflow. LinTunes
  refuses to continue when zero or multiple identifiable devices are present.
- LinTunes records the selected iPod touch UDID before confirmation and checks
  for the same UDID again immediately before launching `idevicerestore`.
- Disk-mode firmware flashing is disabled; LinTunes never lets
  `ipodpatcher` auto-select a disk.
- If LinTunes reports that a restore tool is missing, install
  `idevicerestore` and the `idevice_id` utility, then restart LinTunes.
- If permission is denied, configure your distribution's USB/device access
  rules for the restore tool. Do not run the entire LinTunes application as root.
...
LinTunes rips audio CDs using libcdio-paranoia, a library that uses error-correction algorithms to produce bit-perfect rips even from scratched discs.
...
These command-line tools must be installed for encoding to work. They are installed automatically by `buildPod.sh` and are listed in the dependency section of this manual.

---

## 13. CD Burning

LinTunes burns audio CDs from tracks in your library using `wodim` (a standard Linux CD-burning tool).
...
- All audio formats supported by LinTunes can be burned — they are converted to WAV internally before being handed to `wodim`.
...
### Using the SuperDrive in LinTunes

Once unlocked, the SuperDrive appears as `/dev/sr0` (or `/dev/sr1` if another drive is already present) and LinTunes detects it automatically alongside any internal drives. All ripping and burning features work identically to any other CD drive.
...
| Path | Purpose |
|------|---------|
| `~/.local/share/lintunes/library.db` | SQLite library database (tracks, playlists) |
| `~/Music/` | Default output folder for CD ripping |
| `/tmp/lintunes_rip_*.wav` | Temporary WAV files created during CD ripping (deleted automatically) |
| `/tmp/lintunes_burn_*.wav` | Temporary WAV files created during CD burning (deleted automatically) |

The library database contains only metadata and file paths — your audio files are never moved or copied by LinTunes (except when syncing to an iPod or ripping a CD, where new files are intentionally created).
...
```
If that also has no sound, the issue is with your system audio (PulseAudio/PipeWire), not LinTunes.
...
- Some iPod models require the filesystem to be unlocked with iTunes on Windows or macOS before LinTunes can access them.
...
This error appears if LinTunes tries to probe a restricted kernel filesystem. It is harmless and is suppressed automatically — LinTunes will still scan the correct removable-media locations. If it persists, make sure your user account has read access to your media mount directories:
```bash
ls /run/media/$USER/
```
