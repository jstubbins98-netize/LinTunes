# WinTunes User Manual

WinTunes is a full-featured music manager for Linux, designed to work and feel like iTunes. It manages your music library, plays audio, syncs to iPods, rips and burns CDs, and supports the Apple SuperDrive.
...
WinTunes has a native Windows build using MSYS2 UCRT64. Install MSYS2 from
<https://www.msys2.org>, open an **MSYS2 UCRT64** terminal, and run
`./msys2-build.sh` from this directory (or run `.\buildWindows.ps1` from
PowerShell). The resulting executable is `build-windows/WinTunes.exe`.
Both scripts pass `-DWINTUNES_ENABLE_IPOD=OFF`,
`-DWINTUNES_ENABLE_CD=OFF`, and `-DWINTUNES_ENABLE_RADIO=OFF`; these are
deliberate defaults for the standard MSYS2 installation and can be enabled
individually on platforms where the corresponding libraries and device APIs
are supported.

The database is stored in `%LOCALAPPDATA%\WinTunes\library.db`; the default
library is the Windows Known Folder **Music**. UTF-8 file names are supported.
MSYS2 does not currently provide the libgpod/libcdio stack used by the Linux
build, therefore iPod actions and physical CD ripping/burning report an
explicit unsupported error on Windows. Library management, playlists, TagLib
metadata, GStreamer playback, and other available features continue to work.

### Option A — Automated Windows build

The MSYS2 script installs the UCRT64 dependencies and compiles the program:
...
For Linux, use the manual CMake instructions below and the distribution package
lists in the README.
...
When WinTunes starts for the first time, the library will be empty and a welcome screen is shown with a large **+ Add Music** button. The library database is created automatically at `~/.local/share/wintunes/library.db`. Nothing is imported automatically — you choose what goes in your library.
...
WinTunes never moves or copies your files — it adds references to wherever your audio files already live on disk.
...
- **Add Folder…** — opens a folder chooser. WinTunes scans the selected folder and all sub-folders recursively, importing every audio file it finds.
...
Drag audio files or folders from your file manager directly onto the track list. WinTunes accepts them and imports them in the background. You can drop multiple files and folders at the same time.

### What happens during import

- WinTunes reads the embedded tags (title, artist, album, genre, year, track number, disc number, composer, album artist, BPM, comment) using TagLib.
...
| **Remove from Library** | Removes the track(s) from WinTunes but leaves the audio file on disk |
...
WinTunes supports all classic iPods (iPod classic, iPod mini, iPod nano, iPod photo) using the libgpod library.
...
### Step 2 — Connect in WinTunes

Go to **Device → Connect iPod…**

WinTunes automatically scans `/proc/mounts`, `/media`, `/mnt`, and `/run/media` for a directory containing an `iPod_Control` or `iTunes_Control` folder. If exactly one iPod is found, it connects automatically. If multiple are found, a dialog lets you choose which one.
...
4. When all files are copied, WinTunes writes the iPod database automatically.
...
1. Install `idevicerestore` and `idevice_id` using your distribution's package
   manager.
2. Obtain an Apple-signed IPSW for the exact iPod touch model. WinTunes does not
...
7. Do not unplug the iPod until WinTunes reports that the restore finished.

WinTunes records the selected UDID, verifies that same device again immediately
...
binds the destructive write to the iPod selected in WinTunes. WinTunes refuses
...
iPod. A full firmware-partition restore must be performed outside WinTunes with
...
1. Mount and connect the disk-mode iPod normally in WinTunes.
2. Choose **Device → Erase and Reset Music Database…**.
3. Verify the displayed mount point.
4. Type `ERASE MUSIC` exactly and confirm.

WinTunes identifies the model through libgpod, deletes only the iPod control
...
Immediately before deleting anything, WinTunes reopens the iPod database and
...
one. Older PC/FAT-formatted iPods may not provide a UUID, so WinTunes does not
...
- Connect exactly one iPod touch before opening the restore workflow. WinTunes
  refuses to continue when zero or multiple identifiable devices are present.
- WinTunes records the selected iPod touch UDID before confirmation and checks
  for the same UDID again immediately before launching `idevicerestore`.
- Disk-mode firmware flashing is disabled; WinTunes never lets
  `ipodpatcher` auto-select a disk.
- If WinTunes reports that a restore tool is missing, install
  `idevicerestore` and the `idevice_id` utility, then restart WinTunes.
- If permission is denied, configure your distribution's USB/device access
  rules for the restore tool. Do not run the entire WinTunes application as root.
...
WinTunes rips audio CDs using libcdio-paranoia, a library that uses error-correction algorithms to produce bit-perfect rips even from scratched discs.
...
These command-line tools must be installed for encoding to work and are listed
in the dependency section of this manual.

---

## 13. CD Burning

WinTunes burns audio CDs from tracks in your library using `wodim` (a standard Linux CD-burning tool).

### Requirements

- A blank CD-R disc (standard 74-minute / 650 MB).
- `wodim` installed (`sudo apt install wodim`).
- `ffmpeg` installed (to convert non-WAV formats to the raw PCM that CD burning requires).
...
- All audio formats supported by WinTunes can be burned — they are converted to WAV internally before being handed to `wodim`.
...
### Using the SuperDrive in WinTunes

Once unlocked, the SuperDrive appears as `/dev/sr0` (or `/dev/sr1` if another drive is already present) and WinTunes detects it automatically alongside any internal drives. All ripping and burning features work identically to any other CD drive.
...
| Path | Purpose |
|------|---------|
| `~/.local/share/wintunes/library.db` | SQLite library database (tracks, playlists) |
| `~/Music/` | Default output folder for CD ripping |
| `/tmp/wintunes_rip_*.wav` | Temporary WAV files created during CD ripping (deleted automatically) |
| `/tmp/wintunes_burn_*.wav` | Temporary WAV files created during CD burning (deleted automatically) |

The library database contains only metadata and file paths — your audio files are never moved or copied by WinTunes (except when syncing to an iPod or ripping a CD, where new files are intentionally created).
...
| libcdio | CD device access and TOC reading |
| libcdio-paranoia | Error-corrected CD audio ripping |
| libcddb | CDDB/GnuDB metadata lookup |
...
| `wodim` | CD burning | `wodim` or `cdrtools` |
...
```
If that also has no sound, the issue is with your system audio (PulseAudio/PipeWire), not WinTunes.
...
- Some iPod models require the filesystem to be unlocked with iTunes on Windows or macOS before WinTunes can access them.
- PC/FAT-formatted iPods are supported. Volume labels containing spaces are
  decoded automatically when WinTunes reads the Linux mount table. WinTunes
...
This error appears if WinTunes tries to probe a restricted kernel filesystem. It is harmless and is suppressed automatically — WinTunes will still scan the correct removable-media locations. If it persists, make sure your user account has read access to your media mount directories:
```bash
ls /run/media/$USER/
```
