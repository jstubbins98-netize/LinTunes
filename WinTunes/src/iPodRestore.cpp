...
bool iPodRestore::commandAvailable(const std::string&) { return false; }
bool iPodRestore::validFirmwareFile(iPodRestoreKind kind, const std::string& path,
                                    std::string& error) {
    std::error_code ec;
    ...
    error = "iPod restore tools are not supported on Windows.";
    return false;
}
...
iPodRestoreResult iPodRestore::restore(iPodRestoreKind, const std::string&,
                                        const iPodRestoreTarget&, RestoreOutputCallback) {
    return {false, -1, "iPod restore tools are not supported on Windows."};
}
#else
#include <algorithm>
#include <array>
...
std::vector<std::string> iPodRestore::commandArguments(
    ...
    // bind the write to WinTunes' selected iPod. Never construct a command
...
bool iPodRestore::targetStillMatches(iPodRestoreKind kind,
    ...
    error = "Disk-mode firmware restore is disabled because ipodpatcher cannot "
            "bind its destructive write to the iPod selected in WinTunes.";
    return false;
}
