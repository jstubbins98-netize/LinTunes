...
required dependencies and prints an install command; it never runs package
installation itself. GTK3, GStreamer, TagLib, and SQLite are required. Internet
radio is enabled only when the libVLC development package is visible through
`pkg-config`. A VLC application or Homebrew cask installation alone does not
necessarily include discoverable development headers; in that case MacTunes
builds normally with Internet Radio disabled.
