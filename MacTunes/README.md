...
radio uses a Homebrew libVLC development package when available. Otherwise,
MacTunes loads libVLC and its plugins directly from `VLC.app` in `/Applications`
or `~/Applications`, so the standard VLC cask installation is sufficient.
