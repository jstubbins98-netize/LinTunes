...
The macOS port has honest feature limitations: libgpod iPod sync is optional,
CD burning via Linux `wodim` is unavailable, and optical-drive support depends
on libcdio and the device exposed by macOS. Linux retains the full device
integration.
...
MacTunes rips audio CDs using libcdio-paranoia, a library that uses error-correction algorithms to produce bit-perfect rips even from scratched discs.
...
MacTunes burns audio CDs from tracks in your library using `wodim` (a standard Linux CD-burning tool).
...
- All audio formats supported by MacTunes can be burned — they are converted to WAV internally before being handed to `wodim`.
