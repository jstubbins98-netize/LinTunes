...
On macOS, use `buildMac.sh`. It checks required Homebrew formulae and prints
safe installation guidance without installing anything automatically. VLC is
optional: Internet Radio uses libVLC from `pkg-config` when available or loads
it directly from `VLC.app` in `/Applications` or `~/Applications`:
...
| libVLC (optional) | Internet radio stream playback; loaded through `pkg-config` or directly from `VLC.app` |
