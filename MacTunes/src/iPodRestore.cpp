...
std::vector<std::string> iPodRestore::commandArguments(
    ...
    // bind the write to MacTunes' selected iPod. Never construct a command
...
#ifdef __APPLE__
    error = "Classic iPod firmware flashing is unavailable on macOS; "
            "use Erase and Reset Music Database for a mounted iPod.";
    return false;
#else
    error = "Disk-mode firmware restore is disabled because ipodpatcher cannot "
            "bind its destructive write to the iPod selected in MacTunes.";
    return false;
#endif
}
