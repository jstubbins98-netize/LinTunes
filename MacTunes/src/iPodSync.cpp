...
#ifndef MACTUNES_HAVE_GPOD
iPodSync::iPodSync() {}
iPodSync::~iPodSync() {}
std::vector<std::string> iPodSync::detectMountPoints() { return {}; }
bool iPodSync::connect(const std::string&) {
    last_error_ = "iPod support is unavailable: libgpod was not found at build time.";
    return false;
}
void iPodSync::disconnect() {}
iPodInfo iPodSync::getInfo() const { return {}; }
std::vector<Track> iPodSync::getIPodTracks() const { return {}; }
bool iPodSync::addTrack(const Track&, SyncProgressCallback) { last_error_ = "iPod support is unavailable."; return false; }
bool iPodSync::removeTrack(const std::string&) { last_error_ = "iPod support is unavailable."; return false; }
bool iPodSync::syncPlaylist(const std::string&, const std::vector<Track>&, SyncProgressCallback) { last_error_ = "iPod support is unavailable."; return false; }
bool iPodSync::writeDatabase() { last_error_ = "iPod support is unavailable."; return false; }
bool iPodSync::resetMusicDatabase(const std::string&) { last_error_ = "iPod support is unavailable."; return false; }
#else
...
static bool looksLikeIPod(const std::string& mp) {
    // libgpod resolves the control directory using the casing actually stored
    // on the device. This matters for PC/FAT iPods whose directory names may
    // not have Apple's usual capitalization when viewed from Linux.
    gchar* control_dir = itdb_get_control_dir(mp.c_str());
    if (control_dir) {
        g_free(control_dir);
        return true;
    }
...
static bool isShuffle(const Itdb_iTunesDB* itdb) {
    if (!itdb || !itdb->device) return false;
...
static bool resolveWholeDisk(const std::string& mount, std::string& devicePath,
    ...
    return true;
#else
    (void)mount; (void)devicePath; (void)deviceNumber;
    return false;
#endif
}
...
#else
        std::vector<std::string>{"/media", "/mnt", "/run/media"};
#endif
...
bool iPodSync::connect(const std::string& mount_point) {
    disconnect();
    last_error_.clear();
    if (access(mount_point.c_str(), R_OK | W_OK) != 0) {
        last_error_ = "The iPod is mounted read-only or is not writable. "
                      "Remount the PC-formatted iPod with write access and try again.";
        return false;
    }
    GError* err = nullptr;
    ...
    // Keep both libgpod objects bound to the actual Linux mount. This is
    // important for Windows/FAT iPods, where volume labels and mount paths can
    // differ from the path recorded when the database was created by iTunes.
    itdb_set_mountpoint(itdb_, mount_point.c_str());
    if (itdb_->device) {
        itdb_device_set_mountpoint(itdb_->device, mount_point.c_str());
        itdb_device_read_sysinfo(itdb_->device);
    }
...
Itdb_Track* iPodSync::trackToGpod(const Track& t) const {
    ...
    // iPod firmware uses these fields to decide whether a database entry is a
    // visible song. Do not rely on the zeroed defaults from itdb_track_new().
    std::string ext = fs::path(t.file_path).extension().string();
    std::transform(ext.begin(), ext.end(), ext.begin(),
                   [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
    if (ext == ".mp3")
        gt->filetype = g_strdup("MPEG audio file");
    else if (ext == ".m4a" || ext == ".aac" || ext == ".alac")
        gt->filetype = g_strdup("AAC audio file");
    else if (ext == ".wav")
        gt->filetype = g_strdup("WAV audio file");
    else if (ext == ".aiff" || ext == ".aif")
        gt->filetype = g_strdup("AIFF audio file");
    else
        gt->filetype = g_strdup("Audio file");

    gt->mediatype    = ITDB_MEDIATYPE_AUDIO;
    gt->visible      = 1;
    gt->checked      = 0;
    gt->time_added   = std::time(nullptr);
    gt->time_modified = gt->time_added;
...
std::string iPodSync::copyFileToIPod(const std::string& src_path, Itdb_Track* gt) const {
    ...
    // Keep this explicit: this flag is serialized into the iTunesDB and older
    // firmware may omit entries that are still marked as not transferred.
    gt->transferred = TRUE;
...
bool iPodSync::syncPlaylist(const std::string& playlist_name,
    ...
    for (int i = 0; i < total; ++i) {
        ...
        if (!gt) {
            ...
            if (copyFileToIPod(t.file_path, gt).empty()) {
                itdb_playlist_remove_track(itdb_playlist_mpl(itdb_), gt);
                itdb_track_remove(gt);
                continue;
            }
        }
...
bool iPodSync::writeDatabase() {
    if (!itdb_) { last_error_ = "Not connected"; return false; }
    const guint32 expected_tracks = itdb_tracks_number(itdb_);

    if (!itdb_start_sync(itdb_)) {
        last_error_ = "libgpod could not start the iPod database sync";
        return false;
    }
    ...
    if (!ok) {
        last_error_ = err ? err->message : "Could not write the iPod music database";
        if (err) g_error_free(err);
        itdb_stop_sync(itdb_);
        return false;
    }
    if (err) {
        g_error_free(err);
    ...
    // Shuffle firmware reads iTunesSD/iTunesShuffle in addition to iTunesDB.
    // Without this file, copied tracks can be absent on PC-formatted Shuffles.
    if (isShuffle(itdb_) && !itdb_shuffle_write(itdb_, &err)) {
        last_error_ = err ? err->message : "Could not write the iPod Shuffle database";
        if (err) g_error_free(err);
        itdb_stop_sync(itdb_);
        return false;
    }
    ...
    if (!itdb_stop_sync(itdb_)) {
        last_error_ = "libgpod could not finalize the iPod database sync";
        return false;
    }

    // FAT32 aggressively caches directory and allocation-table updates. Flush
    // this mounted filesystem so files and databases are committed before the
    // user safely removes the iPod.
#ifdef __APPLE__
    int mount_fd = open(mount_point_.c_str(), O_RDONLY);
#else
    int mount_fd = open(mount_point_.c_str(), O_RDONLY | O_DIRECTORY);
#endif
    if (mount_fd >= 0) {
#ifdef __APPLE__
        if (fsync(mount_fd) != 0)
...
#endif
            std::cerr << "Warning: could not flush iPod filesystem buffers\n";
        close(mount_fd);
    } else {
        sync();
    }

    // Do not report success merely because media files were copied. Parse the
    // database from the FAT volume again and make sure all tracks survived the
    // write. An iPod cannot display tracks that are absent from this database.
    GError* verify_error = nullptr;
    Itdb_iTunesDB* verified = itdb_parse(mount_point_.c_str(), &verify_error);
    if (!verified) {
        last_error_ = verify_error
            ? "The iPod database could not be read after syncing: " +
                  std::string(verify_error->message)
            : "The iPod database could not be read after syncing";
        if (verify_error) g_error_free(verify_error);
        return false;
    }
    const guint32 written_tracks = itdb_tracks_number(verified);
    itdb_free(verified);
    if (verify_error) g_error_free(verify_error);
    if (written_tracks != expected_tracks) {
        last_error_ = "The iPod database verification failed: expected " +
                      std::to_string(expected_tracks) + " tracks but found " +
                      std::to_string(written_tracks);
        return false;
    }
    return true;
}
...
#ifdef __APPLE__
    const bool identity_changed = !expectedUuid.empty() && currentUuid != expectedUuid;
#else
