...
std::string CDManager::buildOutputFilename(const CDInfo& info, int track_num,
    ...
    for (const auto& track : info.tracks) {
        if (track.track_number == track_num && !track.title.empty()) {
            title = track.title;
            break;
        }
    }

    auto sanitise = [](std::string value) {
        for (char& c : value) {
            if (c == '/' || c == '\\' || c == ':' || c == '*' ||
                c == '?' || c == '"') {
                c = '_';
    ...
    std::snprintf(track_prefix, sizeof(track_prefix), "%02d", track_num);
    const std::string filename = std::string(track_prefix) + " - " +
                                 sanitise(title) + "." + format;
