...
MainWindow::MainWindow(Library& library, AudioPlayer& player,
    ...
    gtk_window_set_title(GTK_WINDOW(window_), "MacTunes");
...
#ifndef __APPLE__
    // GTK3's Quartz backend can repeatedly pass a null target name to
    // gdk_atom_intern while registering URI-list destinations. Keep the normal
    // file chooser on macOS and avoid that unstable GTK3/Quartz code path.
...
void MainWindow::updateNowPlaying(const Track& t) {
    ...
    std::string title = "MacTunes – " + t.title;
...
void MainWindow::syncToIPod() {
    ...
    std::thread([this, to_sync = std::move(to_sync), total]() {
        ...
        gdk_threads_add_idle([](gpointer d) -> gboolean {
            auto* result = static_cast<SyncResult*>(d);
            result->self->setStatus(result->message);
            result->self->refreshTrackList(result->self->library_.getAllTracks());
            if (!result->ok) {
                GtkWidget* dialog = gtk_message_dialog_new(
                    GTK_WINDOW(result->self->window_), GTK_DIALOG_MODAL,
                    GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, "%s",
                    result->message.c_str());
                gtk_message_dialog_format_secondary_text(
                    GTK_MESSAGE_DIALOG(dialog), "%s",
                    "The audio files may have copied, but the iPod will remain "
                    "empty until its music database is written successfully.");
                gtk_dialog_run(GTK_DIALOG(dialog));
                gtk_widget_destroy(dialog);
            }
            delete result;
            return FALSE;
        }, new SyncResult{this, final_msg, db_ok});
    }).detach();
}
...
void MainWindow::restoreIPod() {
    ...
    if (kind == iPodRestoreKind::TouchIPSW) {
        ...
        GtkWidget* err = gtk_message_dialog_new(GTK_WINDOW(window_), GTK_DIALOG_MODAL,
            ...
            "guarantee that it will write to the iPod selected in MacTunes.");
        gtk_dialog_run(GTK_DIALOG(err));
        gtk_widget_destroy(err);
        return;
    }
...
void MainWindow::ripCD() {
...
#ifdef __APPLE__
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(format_combo), "AIFF (lossless)");
    ...
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(format_combo), "M4A");
#else
    ...
    if (gtk_dialog_run(GTK_DIALOG(dlg)) == GTK_RESPONSE_OK) {
...
#ifdef __APPLE__
        const char* formats[] = {"aiff", "wav", "aac", "m4a"};
        std::string fmt = formats[std::max(0, std::min(fmt_idx, 3))];
#else
        const char* formats[] = {"flac", "mp3", "ogg", "aac", "wav"};
        std::string fmt = formats[std::max(0, std::min(fmt_idx, 4))];
#endif
        std::string out_dir = folder ? folder :
        ...
        std::thread([this, out_dir, fmt, info]() {
            ...
            std::string rip_error = n == 0 ? cd_.lastError() : std::string();
            // Import ripped tracks
            int imported = n > 0 ? library_.importDirectory(out_dir, true) : 0;
            struct RipResult {
                MainWindow* self;
                int imported;
                std::string error;
            ...
            gdk_threads_add_idle([](gpointer d) -> gboolean {
                auto* result = static_cast<RipResult*>(d);
                result->self->refreshTrackList(result->self->library_.getAllTracks());
                if (result->error.empty()) {
                    result->self->setStatus(
                        "Ripped " + std::to_string(result->imported) +
                        " track(s) and imported to library");
                } else {
                    result->self->setStatus("CD rip failed: " + result->error);
                    GtkWidget* error_dialog = gtk_message_dialog_new(
                        GTK_WINDOW(result->self->window_), GTK_DIALOG_MODAL,
                        GTK_MESSAGE_ERROR, GTK_BUTTONS_OK,
                        "CD rip failed: %s", result->error.c_str());
                    gtk_dialog_run(GTK_DIALOG(error_dialog));
                    gtk_widget_destroy(error_dialog);
                }
                delete result;
                return FALSE;
            }, new RipResult{this, imported, rip_error});
        }).detach();
        return;
    }
    gtk_widget_destroy(dlg);
...
void MainWindow::showAbout() {
    ...
    gtk_about_dialog_set_program_name(GTK_ABOUT_DIALOG(dlg), "MacTunes");
    ...
    const char* authors[] = {"MacTunes Contributors", nullptr};
